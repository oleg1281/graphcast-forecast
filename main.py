...
def add_steps(ds):
    ...
    '''# Сохраняем результат
    ds_out.to_netcdf(f"w:/Postprocesing/Oleh Bedenok/GRAPHCAST/NOAA/dataset_era5/ERA5_2025_5_5.nc", format="NETCDF3_CLASSIC")
...
while True:
    ...
    source_files = [f for f in os.listdir(source_folder) if f.endswith(".nc")]

    # Убираем префикс "pred_" у всех файлов в predictions
    predicted_files = {
        f[5:] for f in os.listdir(predictions_folder)
        if f.startswith("pred_") and f.endswith(".nc")
    }
    ...
    for file in source_files:
        if file not in predicted_files:
            ...
            target_path = os.path.join(target_dataset_folder, file)

            # 🔍 1. Предобработка файла
            try:
                ds = xr.open_dataset(source_path)

                ds = add_steps(ds)

                # Сохраняем во временный путь (можно и сразу в datasets)
                ds.to_netcdf(target_path)
                print(f"✅ Обработан и сохранён в: {target_path}")

                # Закрываем dataset вручную (важно для Windows, чтобы можно было удалить)
                ds.close()
            ...
            # 🧠 Запускаем модель (пример команды)
            try:
                print('запуск модели')
                '''subprocess.run(
                    ["python", "main.py", "--input", target_path],
                    cwd=r"c:/Users/obedenok/PycharmProjects/Graphcast",  # директория модели
                    check=True
                )'''
                main_run(file)
                print(f"🎯 Прогноз успешно запущен для {file}")

                # Удаляем исходный файл
                os.remove(target_path)
                print(f"🗑️ Удалён файл: {target_path}")

            except subprocess.CalledProcessError as e:
                print(f"❌ Ошибка при запуске модели для {file}: {e}")
