...
def join_files(date):
    # Загружаем файлы
    surface1 = xr.open_dataset(f"temp/era5_surface_part12.nc", decode_timedelta=True)
    surface2 = xr.open_dataset(f"temp/era5_surface_part22.nc", decode_timedelta=True)
    pressure = xr.open_dataset(f"temp/era5_pressure_part12.nc", decode_timedelta=True)
    ...
    # заменить / на _
    replaced = date.replace('/', '_')

    # Сохраняем в файл
    #output_file = f"temp/out_file2.nc"
    ds_merged.to_netcdf(f"w:/Postprocesing/Oleh Bedenok/GRAPHCAST/NOAA/TEST_Graphcast_14,05,2025-31,05,2025/dataset_ERA5/out_file_{replaced}.nc", format="NETCDF3_64BIT")

    print(f"✅ Готово: данные расширены и сохранены в datasets/out_file_{replaced}.nc")


def add_targets(date):
    # Загружаем исходный файл
    ds = xr.open_dataset("temp/out_file2.nc", decode_timedelta=True)
    ...
    # Сохраняем результат
    ds_out.to_netcdf(f"w:/Postprocesing/Oleh Bedenok/GRAPHCAST/NOAA/TEST_Graphcast_14,05,2025-31,05,2025/dataset_ERA5/out_file_for_graphcast_{replaced}_dell.nc", format="NETCDF3_64BIT")
    print(f"✅ Готово: данные расширены и сохранены в datasets/out_file_for_graphcast_{replaced}.nc")
...
dict_keys = ['30/05_06', '30/05_18', '31/05_06', '31/05_18']
...
for date in dict_keys:
    ...
    dt = datetime(year=2025, month=month, day=day, hour=hour)

    # Получаем дату на 6 часов раньше и на 12 часов раньше
    dt_minus_6 = dt - timedelta(hours=6)
    dt_minus_12 = dt - timedelta(hours=12)

    day_str = dt.strftime("%d")
    month_str = dt.strftime("%m")
    times_str = [dt_minus_12.strftime("%H:%M"), dt_minus_6.strftime("%H:%M")]

    print(f"Минус 6 часов от {date}:", f'day_str = {day_str}, month_str= {month_str}, times_str= {times_str}')

    download_surface_part1("temp/era5_surface_part12.nc", day_str, month_str, times_str)
    change_part1("temp/era5_surface_part12.nc")

    download_surface_part2("temp/era5_surface_part22.nc", day_str, month_str, times_str)
    change_part2("temp/era5_surface_part22.nc")

    download_pressure_part1("temp/era5_pressure_part12.nc", day_str, month_str, times_str)
    change_pressure_part1("temp/era5_pressure_part12.nc")
