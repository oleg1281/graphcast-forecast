...
c = cdsapi.Client(
    ...
    key='8263cc1a-32f2-4e97-a3aa-e0b4f24b7107'
)
...
def join_files():
    # Загружаем файлы
    surface1 = xr.open_dataset(f"delete/era5_surface_part11.nc", decode_timedelta=True)
    surface2 = xr.open_dataset(f"delete/era5_surface_part21.nc", decode_timedelta=True)
    pressure = xr.open_dataset(f"delete/era5_pressure_part11.nc", decode_timedelta=True)
    ...
    # Сохраняем в файл
    output_file = f"delete/out_file1.nc"
    ds_merged.to_netcdf(output_file, format="NETCDF3_64BIT")
...
def add_targets(date):
    ...
    ds = xr.open_dataset("delete/out_file1.nc", decode_timedelta=True)
    ...
    # Сохраняем результат
    ds_out.to_netcdf(f"w:/Postprocesing/Oleh Bedenok/GRAPHCAST/datasets_025_13/out_file_for_graphcast_{replaced}.nc", format="NETCDF3_64BIT")
...
for date in dict_keys:
    ...
    download_surface_part1("delete/era5_surface_part11.nc", day_str, month_str, times_str)
    change_part1("delete/era5_surface_part11.nc")

    download_surface_part2("delete/era5_surface_part21.nc", day_str, month_str, times_str)
    change_part2("delete/era5_surface_part21.nc")

    download_pressure_part1("delete/era5_pressure_part11.nc", day_str, month_str, times_str)
    change_pressure_part1("delete/era5_pressure_part11.nc")
